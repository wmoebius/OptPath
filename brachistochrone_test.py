...
y0 = 0.05
theta0 = np.arccos(1-old_div(y0,b))
...
# point in this case.
thetap = np.arccos(1-old_div(np.array(yp),b))
...
thetap2 = np.array(list(map(find_theta, xp)))
...
arc_length = old_div(4,np.pi*(1-np.cos(old_div(np.pi,2))))
dx=x[1:]-x[:-1]
dy=y[1:]-y[:-1]
...
tt= lambda a : old_div(2*a*np.sin(a/2.0),np.sqrt(np.pi-np.pi*np.cos(a)))
tt0 = tt(np.pi) # total travel time along Brachistochrone curve
# how to use this to check the accuracy of travel_time???

ybar=(y[1:]+y[:-1])/2.0
ntt=sum(old_div(np.sqrt(dx**2+dy**2),np.sqrt(ybar)))
